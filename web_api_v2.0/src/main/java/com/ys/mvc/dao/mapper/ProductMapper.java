...
public interface ProductMapper {
    ...
    @Insert("insert into product values (#{id}, #{type}, #{title}, #{authorId}, #{cover}, #{content}," +
            " #{resourcesTypeId}, #{articleId}, #{discount}, #{updateTime}, #{haveAttachment}, #{attachmentURL}" +
    ...
    @Update("update product set type = #{type}, title = #{title}, authorId = #{authorId}, cover = #{cover}," +
            ...
            " discount = #{discount}, updateTime = #{updateTime}, haveAttachment = #{haveAttachment} " +
            " , attachmentURL = #{attachmentURL}, attachmentPassword = #{attachmentPassword}" +
            ", director = #{director}, directorPhone = #{directorPhone} where id = #{id}")
    int update(Product product);

    List<Product> getList(Pageable<Product> pageable);

    @Select("select count(*) from product p where p.type = #{param1}")
    int count(String type);

    @Delete("delete p from product p where p.id = #{id}")
    int delete(Product product);

    @Select("select p.id, p.cover, p.type, p.title from product p where p.articleId = #{param1} order by p.updateTime desc")
