...
public interface UserMapper {
    ...
    //@Select("select * from user u, role r where r.code = #{object.code} and r.id = u.roleId order by u.registerTime DESC limit #{start}, #{paging}")
    ...
    @Insert("insert into user values (#{id}, #{name}, #{username}, #{password}, #{head}, #{lastTime}," +
            " #{registerTime}, #{roleId}, #{sex}, #{phone}, #{province}, #{city}, #{style}, #{jobTypeId}) ")
    int save(User user);
    ...
    @Update("update user set name = #{name}, username = #{username}, password = #{password}, head = #{head}" +
            ...
            ", phone = #{phone}, province = #{province}, city = #{city}, style = #{style}, jobTypeId = #{jobTypeId} where id = #{id} ")
